;; $Id: dbttlpg.dsl,v 1.1 1999/08/20 09:13:57 gray Exp $
;;
...
(mode book-titlepage-recto-mode
  ...
  (element (author affiliation)
    (let ((jobtitle (select-elements (children (current-node)) 
				     (normalize "jobtitle")))
	  (orgname  (select-elements (children (current-node)) 
				     (normalize "orgname")))
	  (orgdiv   (select-elements (children (current-node)) 
				     (normalize "orgdiv")))
	  (address  (select-elements (children (current-node)) 
				     (normalize "address"))))
      (make display-group
	space-after: (* (HSIZE 2) %head-after-factor% 4)
	(process-node-list jobtitle)
	(process-node-list orgname)
	(process-node-list orgdiv)
	(process-node-list address))))

  (element (affiliation jobtitle)
    (make paragraph
      font-family-name: %title-font-family%
      font-weight: 'bold
      font-size: (HSIZE 1)
...
(mode article-titlepage-recto-mode
  ...
  (element subtitle
    (make paragraph
      ...
      space-before: (* (HSIZE 2) %head-before-factor% 0.5)
      space-after: (if (last-sibling?)
		       (* (HSIZE 2) %head-after-factor%)
		       0pt)
      quadding: %division-subtitle-quadding%
      keep-with-next?: #t
      (process-children)))
  ...
  (element corpauthor
    (make paragraph
      font-family-name: %title-font-family%
      font-posture: 'italic
      space-after: (* %bf-size% %head-after-factor%)
      quadding: 'center
      keep-with-next?: #t
      (process-children)))
  ...
  (element (affiliation jobtitle)
    (make paragraph
      ...
      line-spacing: (* %bf-size% 0.8 %line-spacing-factor%)
      quadding: 'center
      keep-with-next?: #t
      (process-children)))
  ...
  (element (affiliation address email)
    ;; avoid the normal shrinking of verbatim text, it's small enough already
    (make sequence
