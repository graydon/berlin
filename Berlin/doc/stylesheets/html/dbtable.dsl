;; $Id: dbtable.dsl,v 1.1 1999/08/20 09:13:56 gray Exp $
;;
...
(define (cell-relative-colwidth cell relative)
  (let* ((tgroup (find-tgroup cell)))
    (let loop ((colspecs (select-elements (children tgroup) (normalize "colspec")))
	       (reltotal 0))
...
(define (empty-cell? entry) 
  ...
  (let loop ((nl (children entry)))
	...
	#t
	(if (not (equal? (node-property 'class-name (node-list-first nl)) 'pi))
	    #f
